...
*Описание для MRA 5.0 (uussuussssus)*
...
| LPS        | `Пример`             | Имя пользователя. Если его нет, используется юзернейм |
| UL         | `00 00 00 00`        | Авторизован ли контакт. Равен единице или нулю |
| UL         | `01 00 00 00`        | Статус контакта. Клиент игнорирует это поле, если контакт неавторизован |
| LPS        | `79005678943`        | Номер телефона (в случае отсутствия отправляется пустой LPS) |
| LPS        | `STATUS_ONLINE`      | Значение xстатуса |
| LPS        | `Онлайн`             | Заголовок xстатуса |
| LPS        | N/A                  | Описание xстатуса (пустой lps в случае отсутствия) |
| UL         | `00 00 00 00`        | ??? (но при xstatus должен равняться `DF 02 00 00`) |
...
Начиная с MRIM 1.15, появились Xstatus, однако для классических статусов всё равно в тип Xstatus нужно проставлять свои значения вместе с обычным статусом:
